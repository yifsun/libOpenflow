...
func (s *Stats) UnmarshalBinary(data []byte) (err error) {
	...
	for n + 2 < int(s.Length) {
		...
		case XST_OFB_IDLE_TIME:
			klog.V(4).Infof("Received TimeStatField n:%d", n)
			f = new(TimeStatField)
		case XST_OFB_FLOW_COUNT:
			klog.V(4).Infof("Received FlowCountStatField n:%d", n)
			f = new(FlowCountStatField)
		case XST_OFB_PACKET_COUNT:
			fallthrough
		case XST_OFB_BYTE_COUNT:
			klog.V(4).Infof("Received PBCountStatField n:%d", n)
			f = new(PBCountStatField)
		default:
			klog.V(4).Infof("Received Unknown field: %d", data[n+2])
			err = errors.New("Unknown type received for the Stats Field")
			return
		}
		err = f.UnmarshalBinary(data[n:])
		if err != nil {
			klog.V(4).Infof("Failed to unmarshal Stats's Field: err = %v data = %v", err, data[n:])
			return
		}
		n += int(f.Len())
